...
func ReadZip(zipResp *http.Response) ([]string, map[string][]byte) {
         fmt.Println("Processing zip file...")
     zipBytes, err := io.ReadAll(zipResp.Body)
        if err != nil {
            fmt.Sprintf("Failed to read zip file: %v", err)
        
        }

        if len(zipBytes) == 0 {
            fmt.Println("Zip file is empty")
        }

        zipReader, err := zip.NewReader(bytes.NewReader(zipBytes), int64(len(zipBytes)))
        if err != nil {
            http.Error(w, fmt.Sprintf("Failed to read zip archive: %v", err)
        }
