...
func (kv *KeyValueStore) Get(key string) ([]byte, bool) {
	kv.mu.Lock()
	defer kv.mu.Unlock()
	kv.calls = append(kv.calls, Call{Tag: "get", Val: key})
	val, ok := kv.store[key]
	return val, ok
}

func (kv *KeyValueStore) Calls() []Call {
	kv.mu.Lock()
	defer kv.mu.Unlock()
	return kv.calls
}
...
func NewMockRequest(pathWithQuery string) *MockRequest {
	return &MockRequest{
...
// Simulate the `handle` function
func handle(req *MockRequest, resp *MockResponse, cache *KeyValueStore) {
	parsedURL, _ := url.Parse(req.PathWithQuery)
	query := parsedURL.Query()
	userID := query.Get("user_id")
	if userID == "" {
		resp.WriteHeader(http.StatusBadRequest)
		resp.Write([]byte(`{"error":"user_id is required"}`))
		return
	}

	data, found := cache.Get(userID)
	if !found {
